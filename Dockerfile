...
ARG S6_ARCH=amd64
ARG MLAPI_VERSION=remove_globals
ARG PYZM_VERSION=remove_globals
...
RUN   set -x \
      && apt-get install -y \
         ...
         libevdev2 \
         libgeos-dev \
      && python3 -m pip install "git+https://github.com/baudneo/neo-pyzm.git@${PYZM_VERSION}" \
      && mkdir /mlapi_default \
      && mkdir /mlapi \
      && cd /mlapi_default \
      && git clone https://github.com/baudneo/neo-mlapi.git . \
